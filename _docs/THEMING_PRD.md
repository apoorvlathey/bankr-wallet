...
export interface ThemeTokens {
  ...
  colors: {
    ...
    accent: {
      ...
      secondary: string; // Supporting actions / links
      highlight: string; // Attention / callouts
    };
...
storage contract unless the theme is intentionally being promoted as a shipped
option.
...
- **`stripIconColor` inside vivid filled status circles needs `useTheme()`.**
  ...
  as `numericColor` in Phases 7 & 8: a per-component `useTheme()` and an
  `isDarkThemeId(themeId) ? "fg.inverse" : "white"` literal. `TxDetailModal` is
...
- **`status.warning.tint?: string`** — soft tinted warning surface (optional fourth field on `StatusColor`). Bauhaus = `#FFF8DC` cornsilk wash; dark themes = `surface.sunken`. Used by the cross-dapp batch screen page bg, the gas-estimate fallback row, and replaces per-component dark-theme ternaries.
