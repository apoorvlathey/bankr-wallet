...
- ERC-7715 requests (`wallet_getSupportedExecutionPermissions`, `wallet_getGrantedExecutionPermissions`, `wallet_requestExecutionPermissions`) route through `erc7715PermissionHandlers.ts`. Permission requests preflight local-signer account type, request shape, permission/rule allowlists from `erc7715PermissionRegistry.ts` (native allowance, native periodic, native stream, ERC-20 allowance, ERC-20 periodic, ERC-20 stream, token approval revocation), WalletChan-owned caveat derivation from `erc7715PermissionCaveats.ts`, `from`/selected-account consistency, supported chain/RPC availability, and live EIP-7702 delegation to `EIP_7702_DEFAULT_DELEGATE`. Injected requests resolve the sender tab account with `getTabAccount(tabId)` before preflight/listing; WalletConnect requests resolve the session-authorized account. Approval signs only WalletChan-constructed ERC-7710 typed data after user confirmation, persists scoped grants in `erc7715PermissionGrants`, and returns only active origin/account/chain-scoped grants from `wallet_getGrantedExecutionPermissions`.
...
- If the user edits an adjustable request in the confirmation UI, `lib/erc7715PermissionEditing.ts` keeps fixed identity fields immutable (chain, account, delegate, permission type, token, adjustment policy), allows any positive bounded amount, allows periodic frequency edits, allows stream rate / initial allowance / max allowance edits with expiry required and `maxAmount > initialAmount`, allows expiry removal/change for non-stream adjustable requests, allows an earlier start only when the originally requested start is already in the past, and keeps token-approval-revocation method flags immutable while allowing only expiry adjustment. Locked requests cannot change. Confirmation then re-runs ERC-7715 preflight and recomputes caveats from the edited request before signing.
- The registry rejects ambiguous extras, unbounded non-stream amounts, periodic durations over ten years, streams without expiry, stream max caps that do not exceed initial allowance, token approval revocation without an expiry, token approval revocation without at least one enabled method, broad `permit2InvalidateNonces` revocation, malformed token addresses, expired/duplicate expiry rules, missing or invalid start times, start times after expiry, and oversized/ambiguous justification metadata. Permit2 revocation primitives additionally require a WalletChan built-in chain with live code at the canonical Permit2 address on the configured RPC. `permission.justification` is display-only and normalized out of `permission.data` before caveat derivation.
...
- The ERC-7715 enqueue path must synchronize `erc7715PermissionLock.ts` from the saved pending-request list before releasing the in-memory request lock. Do not rely only on `chrome.storage.onChanged`; that event can arrive after the handler returns and briefly reopen external request processing.
- ERC-7715 approval/rejection/timeout results are delivered through `erc7715PermissionResult:{id}` instead of long-lived `sendMessage` channels. Injected dapps create the request id in `inject.ts` and wait on that storage key; WalletConnect stores kind `erc7715Permission` in `walletConnectPendingRequests` and responds to the relay when the same result key is written.
...
- `walletConnectPendingRequests` contains only request routing metadata. It is consumed when a terminal `txResult:{id}`, `sigResult:{id}`, or `erc7715PermissionResult:{id}` is written, then the service worker responds to the dapp over WalletConnect.
