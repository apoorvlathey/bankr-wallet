...
| `txHistory` | Legacy `CompletedTransaction[]` | Read only as the idempotent migration source for IndexedDB `walletchan-history`. Every valid entry is compacted before import; the key is removed only after all imports commit. It is not written by current code. | v1.0.0; migrated next |
