...
| Key             | Shape                                                          | Description                                                                                                                                                                                                                          | Introduced |
| --------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| `accounts`      | `Account[]` — `{ id, type, address, displayName?, createdAt }` | All account metadata. Types: `bankr`, `privateKey`, `seedPhrase`, `impersonator`, `safe`. A `safe` row is non-secret and its verified chain-scoped authority lives in `safeAccounts`. Array order is the user-defined display order used by the wallet account picker and dapp connection selector. Safe is additive; existing users need no rewrite. | v1.0.0; Safe next |
