...
| `privacyVault`            | `{ version: 1, keyId, revision, createdAt, derivation, masterWrappedKey?, passkeyWrappedKey?, keyCheck, recovery }` | Local-only Privacy Pools recovery vault. At least one wrapper is required. `recovery` is `null` until first Shield access, then contains one 12-word BIP39 phrase encrypted by a dedicated random privacy key with key-ID-bound AES-GCM AAD. Normal setup stores independent main-password and passkey wrappers. An existing biometric factor that predates Shield may create a passkey-only empty scaffold on its next assertion so first Shield access needs no password; password rotation or verified factor removal adds the master wrapper before deleting the passkey wrapper. Agent/general/mnemonic keys cannot decrypt it. The exact decoder rejects unknown fields, malformed base64, wrong cryptographic sizes, wrapper-free records, and oversized ciphertext. | next |
...
| `txHistory`                | `CompletedTransaction[]` — `{ id, status, tx, origin, chainName, chainId, txHash, ..., userOperationHash?, feePaymentToken?, broadcastUncertain?, clearSignedMeta?, batchCallOrigins?, forceInclusionMeta?, bridge?, assetChanges?, destAssetChanges?, privacyShieldMeta? }` | Completed transaction history. Max 50 entries. Token-funded ERC-4337 rows add `userOperationHash` while pending and `feePaymentToken` containing the exact selected catalog symbol; both are optional so old rows remain valid. `broadcastUncertain` is an optional additive marker for a locally signed transaction whose deterministic hash is known but whose raw-send response was lost; receipt polling retains it as pending instead of declaring it dropped until an RPC observes the hash, then clears the marker. Old rows omit it and keep the existing dropped-transaction behavior. `forceInclusionMeta` remains additive: missing `protocol` means the released OP Stack shape, while `protocol: "arbitrum"` may also persist the child hash, Inbox/Bridge/SequencerInbox addresses, Bridge delivery preimage, authoritative force deadline, and optional force transaction hash. These public transaction values let recovery reconstruct and revalidate `forceInclusion`; no serialized child transaction or key material is stored. `clearSignedMeta` (optional, added v3.7.1) snapshots the clear-signed summary at submission time — `{ kind: "approve"\|"transfer"\|"nativeSend"\|"erc7730", amount?, tokenSymbol?, tokenLogo?, tokenAddress?, isInfinite?, counterparty?, counterpartyLabel?, counterpartyEns?, intent?, contractName? }` — so the Activity tab can render "Approved 100 USDC to Uniswap V3 Router" without re-running RPC / eth.sh / ENS lookups on every render. Old entries lacking the field gracefully fall back to the raw `functionName` row. `batchCallOrigins` (optional, additive) is written for cross-dapp batch history entries — one `{ origin, favicon }` per encoded call — so the decoded batch-call list in TxDetailModal can show each contributing dapp instead of the synthetic "Cross-Dapp Batch" origin. Old entries without it fall back to the batch-level `origin/favicon`. `bridge` (optional, additive) marks a cross-chain bridge tx — `{ sourceChainId, sourceTxHash?, destinationChainId, destinationChainName, destinationTxHash?, bungeeStatusCode?, requestHash?, routeName?, receiverAddress?, refundTxHash? }`. Set at submission time on the bridge call entry; `requestHash` now stores Socket's `quoteId` for status lookup, and destination fields fill in as `bridgeStatusPoller` reads Socket `/v3/swap/status`. Old entries without it simply render as plain swaps. `assetChanges` (optional, additive) is the post-confirm snapshot of ERC-20 + native flows for the sender — `{ blockNumber, nativeDelta?, erc20Transfers: [{ token, direction: "in"\|"out", counterparty, amountWei, symbol?, decimals?, logoUrl? }] }` — written by `assetChangesExtractor` from receipt logs + `eth_getBalance` so the Activity modal can render "what actually flowed in/out of my wallet". `destAssetChanges` (same shape) is the bridge-destination leg, populated after `bridge.destinationTxHash` arrives. `privacyShieldMeta` is an additive public-only exact-bound projection `{ version, operationId, state, updatedAt, amountWei, shieldedAmountWei }` used to keep the normal Activity row aligned with the durable Shield lifecycle; it never stores commitment, label, derivation index, note, proof, calldata, or recovery data. Failed and pre-existing entries simply lack these fields. | v1.0.0; force-inclusion, ERC-4337, and Shield projection fields next |
...
- `chrome.storage.local.privacyVault` (optional encrypted Privacy Pools recovery; absence means Shield has not initialized it yet)
