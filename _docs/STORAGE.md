...
| `recentlyReceivedTokens` | `Record<"chainId-address", { chainId, contractAddress, addedAt, symbol?, decimals?, logoUrl?, name? }>` | Tokens the user just received in a confirmed tx but the upstream portfolio API hasn't re-indexed yet. Written by `assetChangesExtractor` after a tx's ERC-20 Transfer logs decode an inbound entry; merged into the portfolio catalog (`loadPortfolioTokenCatalog`) like `customTokens` until the entry expires. Auto-expires per-entry after 5 min — lazy pruning inside `getRecentReceivedTokens`. | next |
