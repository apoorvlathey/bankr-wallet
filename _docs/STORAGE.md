...
| `txHistory` | Legacy `CompletedTransaction[]` | Read only as the idempotent migration source for IndexedDB `walletchan-history`. Every valid entry is compacted before import; additive token-funded ERC-4337 fields (`userOperationHash?`, `feePaymentToken?`) are retained. The key is removed only after all imports commit and is not written by current code. | v1.0.0; migrated next |
