...
| `walletConnectPendingRequests` | `Record<requestId, { id, kind, topic, requestId, method, timestamp }>` | Bridges WalletConnect session requests to normal pending queues. `kind` is `"transaction"`, `"signature"`, or `"erc7715Permission"`. Written when a WC tx/signature/ERC-7715 permission request is enqueued; consumed when `txResult:{id}` / `sigResult:{id}` / `erc7715PermissionResult:{id}` is written so the background can respond to the dapp over WalletConnect. Expired after 30 minutes. Additive; absence means no in-flight WC requests. | next |
