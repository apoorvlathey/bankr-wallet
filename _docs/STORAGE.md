...
| Key             | Shape                                                          | Description                                                                                                                                                                                                                          | Introduced |
| --------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| `accounts`      | `Account[]` — base `{ id, type, address, displayName?, createdAt }`; Ledger adds `{ deviceId, hdPath, hdIndex }` | All account metadata. Types: `bankr`, `privateKey`, `seedPhrase`, `ledger`, `impersonator`. Ledger fields are public address/device-derivation metadata and never include a private key. Array order is the user-defined display order used by the wallet account picker and dapp connection selector. Existing wallets keep their current array order; the additive Ledger type needs no migration. | v1.0.0; Ledger next |
...
| `txHistory` | Legacy `CompletedTransaction[]` | Read only as the idempotent migration source for IndexedDB `walletchan-history`. Every valid entry is compacted before import; additive token-funded ERC-4337 fields (`userOperationHash?`, `feePaymentToken?`) and OP Stack/Arbitrum `forceInclusionMeta?` recovery data are retained. The key is removed only after all imports commit and is not written by current code. | v1.0.0; migrated next |
