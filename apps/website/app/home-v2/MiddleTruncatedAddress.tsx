...
export function MiddleTruncatedAddress({ address }: { address: string }) {
  ...
  return (
    <Flex ref={containerRef} flex={1} minW={0} fontSize="14px" color="inherit" align="center">
      <Text ref={textRef} as="span" fontSize="inherit" color="inherit" whiteSpace="nowrap" overflow="hidden" display="block">
        {display.kind === "full" ? (
          display.text
        ) : (
          <>
            ...
            {display.leftMuted ? (
              <Box as="span" fontWeight="700" opacity={0.52}>
                {display.leftMuted}
              </Box>
            ) : null}
            <Box as="span" fontWeight="700" opacity={0.52}>
