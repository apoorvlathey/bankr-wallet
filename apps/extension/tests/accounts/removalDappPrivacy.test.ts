...
} from "../../src/chrome/accountRemovalDappPrivacy";
...
test("production connection approval and account removal use the shared privacy boundary", async () => {
  const [connectionSource, backgroundSource] = await Promise.all([
    readFile(
      new URL("../../src/chrome/dappConnectionHandlers.ts", import.meta.url),
      "utf8",
    ),
  ...
  const removeStart = backgroundSource.indexOf('case "removeAccount":');
  const revealStart = backgroundSource.indexOf(
    'case "revealPrivateKey":',
    removeStart,
  );
  assert.ok(removeStart >= 0 && revealStart > removeStart);
  const removeRoute = backgroundSource.slice(removeStart, revealStart);
  assert.match(removeRoute, /removeAccountWithDappPrivacyBoundary/);
