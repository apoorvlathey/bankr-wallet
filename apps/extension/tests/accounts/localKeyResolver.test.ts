...
test("local account key resolution supports legacy and vault-key sessions without leaking unrelated keys", async (t) => {
  ...
  try {
    ...
    const resolverModule = await import(
      "../../src/chrome/accounts/localKeyResolver"
    );
    ...
    await t.test("seed-derived accounts resolve through the same account-bound vault path", async () => {
      reset();
      const vaultKeyBytes = cryptoModule.generateVaultKey();
      const vaultKey = await cryptoModule.importVaultKey(vaultKeyBytes);
    ...
    await t.test("non-local account metadata cannot turn ciphertext into a signing capability", async () => {
      reset();
      const vaultKeyBytes = cryptoModule.generateVaultKey();
      const vaultKey = await cryptoModule.importVaultKey(vaultKeyBytes);
      chromeHarness.stores.local.accounts = [{ ...account, type: "bankr" }];
      chromeHarness.stores.local.pkVault = {
        version: 1,
        entries: [
          {
