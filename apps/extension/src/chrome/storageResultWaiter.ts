 ...
 * open. If timeout ownership is ambiguous (for example, confirmation already
 * owns the first-action claim), keep the listener alive and retry expiry. A
 * dapp must never receive a local timeout while WalletChan may still sign or
 * broadcast the same request.
 */
