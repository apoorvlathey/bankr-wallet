import type { DecryptedEntry, Vault, VaultEntry } from "../types";
...
import { retainValidLocalAccountKeys } from "./accountIntegrity";
import {
  decryptPrivateKey,
  encryptPrivateKey,
...
import { createEmptyVault, loadVault, saveVault } from "./repository";
import {
  assertVaultEntryId,
...
export async function addKeyToVault(
  ...
  await withStorageLock(WALLET_SECRET_STORAGE_LOCK_KEY, async () => {
    ...
    // Check if vault key system is active. A migrated wallet must never create
    // a new password-encrypted entry merely because its in-memory vault key
