...
import type { PreflightTokenMetadata } from "./types";
 ...
 * heavier bytecode-injection simulation. If Multicall3 is unavailable, return
 * the original bounded list so previews fail open to the existing simulator.
 */
export async function preflightAssetCandidates(
  ...
  try {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const isErc1155 =
        erc1155?.status === "success" && erc1155.result === true;
