...
export function shouldRetainUnobservedBroadcast(
  tx: Pick<CompletedTransaction, "broadcastUncertain"> | null | undefined,
): boolean {
  ...
  try {
    ...
    if (txData === null) {
      ...
      if (shouldRetainUnobservedBroadcast(tx)) {
        // The initial send response was lost. This RPC not knowing the hash
        // does not prove that another node/load-balancer backend did not
        // accept it, so a "dropped" verdict would invite a duplicate retry.
        notFoundCounts.delete(txId);
        return null;
      }
...
async function fetchTxByHash(
  ...
  return (await fetchRpcResult(rpcUrl, "eth_getTransactionByHash", [txHash], {
...
async function fetchReceipt(
  ...
  return (await fetchRpcResult(rpcUrl, "eth_getTransactionReceipt", [txHash], {
    allowPrivateWithoutOrigin: true,
  })) || null;
}
...
async function buildGasData(
  ...
  if (signedGasLimit !== undefined) {
    ...
    try {
      ...
      if (transaction?.gas) {
        gasLimitStr = BigInt(transaction.gas).toString();
      }
    } catch {
      // Non-critical
    }
