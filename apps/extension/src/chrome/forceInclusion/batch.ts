...
import {
  ...
  type PendingRequestEffectGuard,
...
export function shouldHaltForceInclusionTail(
  ...
  return isBroadcastOutcomeUncertain(result);
}
...
export async function processForceInclusionBatchLocal(
  ...
  const effectGuard = guardPendingRequestEffectLease(effectLease);
  try {
    await processForceInclusionBatchLocalImpl(
      bundleId,
      pending,
      account,
      privateKey,
    ...
    await handleBatchForceInclusionFailure(bundleId, pending, message);
  } finally {
    effectGuard.releaseIfSafe();
  }
}

async function processForceInclusionBatchLocalImpl(
  bundleId: string,
  pending: PendingBatchTxRequest,
  account: { id: string; address: string; type: string },
  privateKey: `0x${string}`,
  ...
  for (let i = 0; i < prepared.length; i++) {
    ...
    try {
      ...
      // Settle only after the deterministic hash and ambiguity marker are
      // durable. A storage failure after raw send must remain fail-closed.
      effectGuard.settleEffect();
      ...
      if (broadcastUncertain) {
        // The current nonce may or may not be in the mempool. Never submit a
        // higher-nonce tail that could execute much later if another tx fills
        // this gap.
        for (let j = i + 1; j < prepared.length; j++) {
          ...
          results.push({ txId: skipped.txId, success: false, error });
        }
        break;
      }
    } catch (err: any) {
  ...
  // Receipt waits can take minutes. All attempted sends now have durable
  // pending/terminal rows, so reset/account maintenance no longer needs to
  // remain blocked during Phase 4.
  effectGuard.releaseIfSafe();
  ...
  await Promise.all(successfulResults.map(async (item) => {
    try {
      ...
      await updateTxInHistory(item.txId, {
        status: "pending",
        txHash: item.l1TxHash,
        broadcastUncertain: item.broadcastUncertain === true,
      ...
      // Do not turn receipt lookup timeout into a definite broadcast failure.
      // Startup recovery will continue reconciling this deterministic hash.
    }
  }));
