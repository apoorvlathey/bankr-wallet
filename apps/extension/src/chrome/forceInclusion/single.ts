...
export function createL1PublicClient(rpcUrl: string): PublicClient {
  return createPublicClient({
    transport: secureHttpTransport(rpcUrl, { timeout: L1_RPC_TIMEOUT, retryCount: 1 }),
  });
}
...
export async function processForceInclusionLocal(
  ...
  attachClearSignedMetaToHistory(
    ...
    // The deterministic hash is now durable. It is safe to release reset and
    // account-mutation exclusion before the potentially long receipt wait.
    effectGuard.settleEffect();
    effectGuard.releaseIfSafe();
    ...
    try {
      ...
      // A known or uncertain broadcast cannot become a definite failure merely
      // because receipt lookup timed out. Keep it recoverable by hash; startup
      // reconciliation will continue checking the L1 transaction.
      await retainPendingForceInclusionBroadcast(
        txId,
        pending,
        info,
        l1Hash,
...
async function retainPendingForceInclusionBroadcast(
  txId: string,
  pending: PendingTxRequest,
  info: ForceInclusionChainInfo,
  l1Hash: string,
