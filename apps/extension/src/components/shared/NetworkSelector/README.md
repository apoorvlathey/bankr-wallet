...
- `model.ts` owns deterministic eligibility-then-funding ordering. Explicitly
  selectable networks stay above disabled networks; within each group, funded
  networks sort by USD value, followed by Ethereum and then all remaining
  unfunded networks alphabetically.
- `index.ts` is the stable cross-feature export used by Swap, Send, the
  homepage portfolio filter, WalletConnect controls, and the dapp dock's chain
  drawer.
