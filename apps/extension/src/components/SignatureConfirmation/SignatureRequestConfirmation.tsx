...
function SignatureRequestConfirmation({
  ...
  const rawOriginHostname = getOriginHostname(trustedOrigin, origin);
  const originHostname = formatOrigin(origin).hostname ?? rawOriginHostname;
  const fallbackFavicon = googleFaviconUrl(rawOriginHostname);
