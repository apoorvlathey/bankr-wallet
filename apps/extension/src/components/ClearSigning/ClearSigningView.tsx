...
export function ClearSigningView(props: ClearSigningViewProps) {
  const { chainId, embedded = false, hideHeader = false, hideLoadingSkeleton } = props;
  ...
  // > 0 we drop the shadow so the border + raised surface do the lifting
  ...
  const cardShadow =
    embedded || (depth > 0 && isDarkThemeId(themeId)) ? "none" : undefined;
  ...
  // Clear signing is the primary request-detail surface, so it uses the same
  // neutral raised layer as the rest of the confirmation hierarchy. The border
  // provides separation without introducing a protocol-colored tint.
  return (
    <ThemedCard
      variant="default"
      weight="thin"
