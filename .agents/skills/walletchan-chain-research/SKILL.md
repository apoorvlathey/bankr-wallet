...
description: Research and verify EVM chain metadata, WalletChan feature support, Safe multisig availability, Pimlico fee-token availability, and notification-safe icon assets for WalletChan chain additions or updates. Use when Codex is asked to add a new WalletChan chain, update chain params, map official testnet chain IDs to a mainnet logo, verify RPC/explorer/native currency/icon, determine Bankr, Safe, swap, bridge, EIP-7702, or token-paid gas support, or keep the extension and website chain allowlists synchronized.
---
...
1. Read the repo guidance first when working inside WalletChan:
   ...
   - `_docs/SAFE_ACCOUNTS.md` when Safe support is relevant
...
5. Read `references/walletchan-fields.md` before editing code. Use it to map
   ...
   website swap API allowlist, Safe's dynamic support policy, and both Pimlico
   fee-token catalogs.
...
7. Treat Safe support as a live capability, not a WalletChan chain-registry
   flag. Match the exact numeric chain ID in Safe's Config Service, require a
   valid official Transaction Service URL, and confirm standard EVM Safe
   compatibility. Do not infer support from deployment addresses or another
   chain in the same ecosystem.
8. Treat Pimlico support as account- and chain-specific live data. Query
...
9. For every verified fee token, update both exact-address allowlists in the
   same change:
...
10. Report uncertainty explicitly. If a source cannot verify a field, leave the
   risky flag false or omit the optional ID instead of guessing.
11. Research current public testnets from the chain operator's documentation and
...
- Safe Config Service: exact chain ID in
  `https://safe-config.safe.global/api/v1/chains/?limit=100`, plus Safe's
  official supported-network and multi-chain-deployment documentation.
...
- Safe account support: exact Config Service match, official Transaction
  Service URL, recommended singleton version, and any EVM exclusion.
